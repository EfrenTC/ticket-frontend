...
<template>
  <div class="min-h-screen bg-black text-white overflow-hidden">
    ...
    <div class="flex items-center justify-center min-h-[calc(100vh-80px)] px-4 py-12">
      <div class="register-card max-w-md w-full rounded-2xl p-8 animate-fade-in">
        ...
        <div class="text-center mb-8">
          <div class="inline-flex items-center gap-2 mb-5 border border-violet-700/40 bg-violet-950/30 px-4 py-1.5 rounded-full">
            ...
            <span class="text-violet-300 text-xs font-medium tracking-wide">Registro de cuenta</span>
          </div>
          <h2 class="text-3xl font-bold text-white tracking-tight">Crea tu cuenta</h2>
          <p class="text-gray-500 text-sm mt-2">Completa los datos para comenzar</p>
        </div>

        <form @submit.prevent="handleSubmit" class="space-y-5">
          <!-- Nombre -->
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Nombre completo</label>
            <input
              v-model="form.name"
              type="text"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Confirmar Contraseña</label>
            <input
          ...
          <button
            type="submit"
            class="submit-btn w-full text-white font-semibold py-3 rounded-xl transition-all duration-300 transform hover:-translate-y-0.5 active:scale-95 mt-2"
          >
            Registrarse
          </button>
        </form>
